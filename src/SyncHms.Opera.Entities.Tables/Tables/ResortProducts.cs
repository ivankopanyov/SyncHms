...
public partial class ResortProducts
{
    public ResortProducts()
    {
    ...
    public virtual PkgForcastGroups PkgForcastGroups { get; set; }
	...
	{
		modelBuilder.Entity<ResortProducts>(entity =>
        {
            entity.HasKey(e => new { e.Resort, e.Product })
                .HasName("RESORT_PRODUCTS_PK");

            entity.ToTable("RESORT_PRODUCTS");

            entity.HasIndex(e => new { e.PkgForcastGroup, e.Resort })
                .HasName("RESORT_PRODUCTS_FK4_IDX");
			...
			if (!types.Contains(typeof(Resort)))
	            ...
	            entity.HasOne(d => d.ResortNavigation)
	                .WithMany(p => p.ResortProducts)
	                .HasForeignKey(d => d.Resort)
	                .OnDelete(DeleteBehavior.ClientSetNull)
	                .HasConstraintName("RESORT_PRODUCTS_FK1");

			if (!types.Contains(typeof(PkgForcastGroups)))
				entity.Ignore(e => e.PkgForcastGroups);
			else
	            entity.HasOne(d => d.PkgForcastGroups)
	                .WithMany(p => p.ResortProducts)
	                .HasPrincipalKey(p => new { p.PkgForcastGroup, p.Resort })
	                .HasForeignKey(d => new { d.PkgForcastGroup, d.Resort })
	                .HasConstraintName("RESORT_PRODUCTS_FK4");
