...
public partial class ArAccountTypes
{
    public ArAccountTypes()
    {
        ArAccount = new HashSet<ArAccount>();
        ArReminderCycle = new HashSet<ArReminderCycle>();
    }
    ...
    public virtual ICollection<ArReminderCycle> ArReminderCycle { get; set; }

	public static void OnModelCreating(ModelBuilder modelBuilder, ISet<Type> types)
	{
		modelBuilder.Entity<ArAccountTypes>(entity =>
        {
            entity.HasKey(e => new { e.Resort, e.AccountTypeId })
                .HasName("ACC_TYPE_PK");

            entity.ToTable("AR$_ACCOUNT_TYPES");

            entity.HasIndex(e => new { e.AccountType, e.Resort })
                .HasName("ACC_TYPE_UK")
                .IsUnique();
			...
			if (!types.Contains(typeof(ArReminderCycle)))
				entity.Ignore(e => e.ArReminderCycle);
		});
	}
}
