...
public partial class ScPkgRevenue
{
    ...
    public virtual ScPkgPrice PkgPriceLinkNavigation { get; set; }

	public static void OnModelCreating(ModelBuilder modelBuilder, ISet<Type> types)
	{
		modelBuilder.Entity<ScPkgRevenue>(entity =>
        {
            entity.HasKey(e => new { e.PkgPriceLink, e.PkgEvLink, e.RevenueType })
                .HasName("PKGREV_PK");

            entity.ToTable("SC$PKG_REVENUE");
			...
			if (!types.Contains(typeof(ScPkgPrice)))
				entity.Ignore(e => e.PkgPriceLinkNavigation);
			else
	            entity.HasOne(d => d.PkgPriceLinkNavigation)
	                .WithMany(p => p.ScPkgRevenue)
	                .HasForeignKey(d => d.PkgPriceLink)
	                .HasConstraintName("PKGREV_PKGPR_FK");
        });
	}
}
