namespace SyncHms.Opera.Entities.Tables;

public partial class BatchProcLog
{
    public string? ProcId { get; set; }
    public decimal? LogLine { get; set; }
    public string? EntryTime { get; set; }
    public string? LogTxt { get; set; }

	public static void OnModelCreating(ModelBuilder modelBuilder, ISet<Type> types)
	{
		modelBuilder.Entity<BatchProcLog>(entity =>
        {
            entity.HasNoKey();

            entity.ToTable("BATCH_PROC_LOG");

            entity.HasIndex(e => e.EntryTime)
                .HasName("BATCH_PROC_I2");

            entity.HasIndex(e => new { e.ProcId, e.LogLine })
                .HasName("BATCH_PROC_LOG_I1");

            entity.Property(e => e.EntryTime)
                .IsRequired()
                .HasColumnName("ENTRY_TIME")
                .HasMaxLength(40)
                .IsUnicode(false);

            entity.Property(e => e.LogLine)
                .HasColumnName("LOG_LINE")
                .HasColumnType("NUMBER");

            entity.Property(e => e.LogTxt)
                .HasColumnName("LOG_TXT")
                .IsUnicode(false);

            entity.Property(e => e.ProcId)
                .IsRequired()
                .HasColumnName("PROC_ID")
                .HasMaxLength(40)
                .IsUnicode(false);
        });
	}
}
