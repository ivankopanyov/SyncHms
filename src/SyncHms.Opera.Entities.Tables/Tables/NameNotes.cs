...
public partial class NameNotes
{
	...
	{
		modelBuilder.Entity<NameNotes>(entity =>
        {
            entity.HasKey(e => new { e.NoteId, e.NameId })
                .HasName("NNOTE_PK");

            entity.ToTable("NAME$NOTES");

            entity.HasIndex(e => e.ExternalNoteId)
                .HasName("NNOTES_EXT_ID_IDX");

            entity.HasIndex(e => e.InactiveDate)
                .HasName("NNOTE_IND1");

            entity.HasIndex(e => e.NameId)
                .HasName("NNOTE_NAME_IDX");

            entity.HasIndex(e => e.NoteTitle)
                .HasName("NNOTE_TITLE_IDX");
			...
			if (!types.Contains(typeof(Name)))
	            ...
	            entity.HasOne(d => d.Name)
	                .WithMany(p => p.NameNotes)
	                .HasForeignKey(d => d.NameId)
	                .HasConstraintName("NNOTE_NAME_FK");
        });
	}
}
