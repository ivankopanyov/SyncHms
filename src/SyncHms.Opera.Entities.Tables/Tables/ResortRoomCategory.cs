...
public partial class ResortRoomCategory
{
    public ResortRoomCategory()
    {
        ResvAutoAttachRules = new HashSet<ResvAutoAttachRules>();
    }
    ...
    public virtual ICollection<ResvAutoAttachRules> ResvAutoAttachRules { get; set; }

	public static void OnModelCreating(ModelBuilder modelBuilder, ISet<Type> types)
	{
		modelBuilder.Entity<ResortRoomCategory>(entity =>
        {
            entity.HasKey(e => new { e.Resort, e.RoomCategory })
                .HasName("RESORT_ROOM_CATEGORY_PK");

            entity.ToTable("RESORT$_ROOM_CATEGORY");

            entity.HasIndex(e => new { e.Resort, e.Label })
                .HasName("RESORT_ROOM_CAT_LABEL_UNIQ")
                .IsUnique();

            entity.HasIndex(e => new { e.Resort, e.RoomClass })
                .HasName("RRCAT_ROOM_CLASS_FK_I");

            entity.HasIndex(e => new { e.Resort, e.YieldCategory })
                .HasName("RRCAT_YC_FK_I");
			...
			if (!types.Contains(typeof(Resort)))
	            ...
	            entity.HasOne(d => d.ResortNavigation)
	                .WithMany(p => p.ResortRoomCategory)
	                .HasForeignKey(d => d.Resort)
	                .OnDelete(DeleteBehavior.ClientSetNull)
	                .HasConstraintName("RESORT_FK20");
			...
			if (!types.Contains(typeof(ResortRoomClasses)))
	            ...
	            entity.HasOne(d => d.R)
	                ...
	                .OnDelete(DeleteBehavior.ClientSetNull)
	                .HasConstraintName("RRCAT_ROOM_CLASS_FK");
			...
			if (!types.Contains(typeof(YieldCategories)))
	            ...
	            entity.HasOne(d => d.YieldCategories)
	                ...
	                .HasConstraintName("RES$_ROOM_CAT_YC_FK");
        
			if (!types.Contains(typeof(ResvAutoAttachRules)))
				entity.Ignore(e => e.ResvAutoAttachRules);
		});
	}
}
