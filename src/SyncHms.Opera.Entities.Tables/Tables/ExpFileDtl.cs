...
public partial class ExpFileDtl
{
	...
	{
		modelBuilder.Entity<ExpFileDtl>(entity =>
        {
            entity.HasKey(e => new { e.ExpFileId, e.ExpFileDtlId })
                .HasName("EFD_PK");

            entity.ToTable("EXP_FILE_DTL");
			...
			if (!types.Contains(typeof(ExpFileHdr)))
	            ...
	            entity.HasOne(d => d.ExpFile)
	                .WithMany(p => p.ExpFileDtl)
	                .HasForeignKey(d => d.ExpFileId)
	                .OnDelete(DeleteBehavior.ClientSetNull)
	                .HasConstraintName("EFD_EFH_FK");
        });
	}
}
