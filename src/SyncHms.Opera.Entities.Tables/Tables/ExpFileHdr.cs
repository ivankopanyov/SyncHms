...
public partial class ExpFileHdr
{
    public ExpFileHdr()
    {
        ExpDataHdr = new HashSet<ExpDataHdr>();
        ExpFileDtl = new HashSet<ExpFileDtl>();
    ...
    public virtual ICollection<ExpDataHdr> ExpDataHdr { get; set; }
    public virtual ICollection<ExpFileDtl> ExpFileDtl { get; set; }
    public virtual ICollection<ExpParameters> ExpParameters { get; set; }

	public static void OnModelCreating(ModelBuilder modelBuilder, ISet<Type> types)
	{
		modelBuilder.Entity<ExpFileHdr>(entity =>
        {
            entity.HasKey(e => e.ExpFileId)
                .HasName("EFH_PK");

            entity.ToTable("EXP_FILE_HDR");

            entity.HasIndex(e => new { e.ParentId, e.Resort })
                .HasName("EXP_FILE_HDR_PARENT_IDX");

            entity.HasIndex(e => new { e.FileType, e.FileGroupId, e.Resort })
                .HasName("EXP_FILE_HDR_IND");
            ...
            if (!types.Contains(typeof(ExpFileDelivery)))
                entity.Ignore(e => e.ExpFileDelivery);

            if (!types.Contains(typeof(ExpDataHdr)))
				entity.Ignore(e => e.ExpDataHdr);

			if (!types.Contains(typeof(ExpFileDtl)))
				entity.Ignore(e => e.ExpFileDtl);

			if (!types.Contains(typeof(ExpParameters)))
				entity.Ignore(e => e.ExpParameters);
		});
	}
}
