...
public partial class ScPkgMenu
{
	...
	{
		modelBuilder.Entity<ScPkgMenu>(entity =>
        {
            entity.HasKey(e => new { e.PkgEvLink, e.MenuId })
                .HasName("PKGMEN_PK");

            entity.ToTable("SC$PKG_MENU");
			...
			if (!types.Contains(typeof(ScPkgEvent)))
	            ...
	            entity.HasOne(d => d.PkgEvLinkNavigation)
	                .WithMany(p => p.ScPkgMenu)
	                .HasForeignKey(d => d.PkgEvLink)
	                .HasConstraintName("PKGMEN_PKGEV_FK");
        });
	}
}
