...
public partial class MembershipIssuedAwards
{
	...
	{
		modelBuilder.Entity<MembershipIssuedAwards>(entity =>
        {
            entity.HasKey(e => e.IssuedAwardId)
                .HasName("AWARDS_ISSUE_PK");

            entity.ToTable("MEMBERSHIP_ISSUED_AWARDS");

            entity.HasIndex(e => e.AwardVoucherNo)
                .HasName("MEMBERSHIP_ISSUED_AWARDS_UK1");

            entity.HasIndex(e => e.BillingGroup)
                .HasName("MEMBERSHIP_ISSUED_AWARDS_IDX6");

            entity.HasIndex(e => e.MembershipId)
                .HasName("MEMBERSHIP_ISSUED_AWARDS_IDX2");

            entity.HasIndex(e => e.NameId)
                .HasName("MEMBERSHIP_ISSUED_AWARDS_IDX1");

            entity.HasIndex(e => e.TrxNo)
                .HasName("MEMBERSHIP_ISSUED_AWARDS_IDX5");

            entity.HasIndex(e => new { e.CancelStatementId, e.CancelMemberStatementId })
                .HasName("MEMBERSHIP_ISSUED_AWARDS_IDX4");

            entity.HasIndex(e => new { e.Resort, e.LocalResvNameId })
                .HasName("MEMBERSHIP_ISSUED_AWARDS_IDX7");

            entity.HasIndex(e => new { e.StatementId, e.MemberStatementId })
                .HasName("MEMBERSHIP_ISSUED_AWARDS_IDX3");

            entity.HasIndex(e => new { e.ChainCode, e.MembershipType, e.AwardType })
                .HasName("MEMBERSHIP_ISSUED_AWARDS_IDX8");
			...
			if (!types.Contains(typeof(MembershipAwards)))
	            ...
	            entity.HasOne(d => d.MembershipAwards)
	                .WithMany(p => p.MembershipIssuedAwards)
	                .HasForeignKey(d => new { d.ChainCode, d.MembershipType, d.AwardType })
	                .OnDelete(DeleteBehavior.ClientSetNull)
	                .HasConstraintName("MEMBERSHIP_ISSUED_AWARDS_FK1");
        });
	}
}
