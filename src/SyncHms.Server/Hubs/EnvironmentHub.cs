...
public class EnvironmentHub(IEnvironmentRepository<ApplicationEnvironment> environmentRepository, ILogger<EnvironmentHub> logger) : Hub
{
    ...
    public async Task Environment()
    {
        await Clients
            ...
            .TrySendAsync("Environment", JsonConvert.SerializeObject(await environmentRepository.GetAsync() ?? new(), new JsonSerializerSettings()
            {
