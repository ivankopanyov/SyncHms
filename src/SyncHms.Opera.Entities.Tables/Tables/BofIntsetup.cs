...
public partial class BofIntsetup
{
	...
	{
		modelBuilder.Entity<BofIntsetup>(entity =>
        {
            entity.ToTable("BOF$INTSETUP");

            entity.HasIndex(e => new { e.BofIntfCode, e.Resort })
                .HasName("INTERFACE_ID_FK_IDX");
			...
			if (!types.Contains(typeof(BofInterface)))
	            ...
	            entity.HasOne(d => d.BofInterface)
	                .WithMany(p => p.BofIntsetup)
	                .HasForeignKey(d => new { d.BofIntfCode, d.Resort })
	                .OnDelete(DeleteBehavior.ClientSetNull)
	                .HasConstraintName("INTERFACE_ID_FK");
        });
	}
}
